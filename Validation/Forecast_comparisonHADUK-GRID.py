...
# Climatology: https://data.ceda.ac.uk/badc/ukmo-hadobs/data/insitu/MOHC/HadOBS/HadUK-Grid/v1.2.0.ceda/5km/rainfall/
import sys
import numpy as np 
import pandas as pd
...
for model_type in range(3):
    ...
    for ind, month_no in enumerate(month_nos):
        ...
        for forecast in forecasts[model_type]:
            filename_hires = f'/ReClimate_{format("%02d" % start_month)}_{start_year}_{str(leads[ind])}_Precipitation-Actuals{forecast}.asc'
        ...
        if netcdf_file:
            # Read analysis month
            datapath_HADUK = f'HADUK-Grid/rainfall_hadukgrid_uk_5km_mon_{years[ind]}{format("%02d" % month_nos[ind])}.nc'
            ...
            monthly_precip = ncc.variables['rainfall'][:]; monthly_precip = monthly_precip[0][:]; monthly_precip = monthly_precip[::-1][:]; monthly_precip = monthly_precip[0:288-44,1:][:]
            monthly_precip[monthly_precip == monthly_precip.fill_value] = np.nan
            # Read climatology month (1991 to 2020)
            datapath_HADUK = 'HADUK-Grid/rainfall_hadukgrid_uk_5km_mon-30y_199101-202012.nc'
            ncc = Dataset(datapath_HADUK, 'r')
            lon = ncc.variables['longitude'][:]; lon = lon[::-1][:]; lon = lon[0:288-44,1:][:]
            lat = ncc.variables['latitude'][:]; lat = lat[::-1][:]; lat = lat[0:288-44,1:][:] # Degrees latitude is a regular spatial (km) grid      
            time = ncc.variables['time']
            monthly_clim = ncc.variables['rainfall'][month_no-1][:]; monthly_clim = monthly_clim[:]; monthly_clim = monthly_clim[::-1][:]; monthly_clim = monthly_clim[0:288-44,1:][:]
        ...
        z_HADUKflat = np.array(monthly_precip).flatten()
    ...
    mask1 = [(np.isnan(HADUKdata_monthX)) | (HADUKdata_monthX > 1000) | (HADUKdata_monthX < 0)][0]
    HADUKdata_monthX = np.where(~mask1, HADUKdata_monthX, np.nan)
    mask2 = [(np.isnan(FCSTdata_scipyX)) | (FCSTdata_scipyX > 1000) | (FCSTdata_scipyX < 0)][0]
    FCSTdata_scipyX = np.where(~mask2, FCSTdata_scipyX, np.nan)
    mask3 = [(np.isnan(benchmarkFCST_scipyX)) | (benchmarkFCST_scipyX > 1000) | (benchmarkFCST_scipyX < 0)][0]
    ...
    ax.set_yticks([])
    ax.set_frame_on(False)
    optional_levels = np.arange(0,320,20)
    ...
    brier_score = xs.brier_score(a, b, np.linspace(0, 1, 11), dim='time')
    ...
    ax.set_yticks([])
    ax.set_frame_on(False)
    brier_clim = xs.brier_score(a, HADUKdata_climX-HADUKdata_climX, np.linspace(0, 1, 11), dim='time')
    ...
    plt.annotate('UK-Wide Brier Score '+mean_brier+',\nValid: '+months[0]+' '+years[0]+' to\n         '+months[-1]+' '+years[-1]+'\nReference: HADUK-Grid',(1000,1000), color='maroon')
    plt.savefig('raw_data/Re-ClimateActuals_HADUK_Grid_Brier-Precip_CLIM.png', dpi=150, bbox_inches='tight')
    plt.show(block=False)
