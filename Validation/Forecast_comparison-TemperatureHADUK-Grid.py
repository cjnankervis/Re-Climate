...
# lines = ax.contour(UK_Lon, UK_Lat, np.ma.array(FCSTdata_scipy), colors=['black']*len(optional_levels), linewidths=[0.5]*len(optional_levels), alpha=0.5)
# ax.xaxis.set_major_formatter(FormatStrFormatter('%.01f'))
fig.colorbar(surf, aspect=8, label='Root-Mean-Square Error', format="%.02f", fraction=0.25)
...
# Save Plot
plt.savefig(f'raw_data/Re-ClimateActuals_HADUK_Grid_Brier{output_ext}.png', dpi=150, bbox_inches='tight')
plt.show(block=False)
